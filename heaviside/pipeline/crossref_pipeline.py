...
def _stage_param_check(state: CrossRefState) -> None:
    ...
    if os.environ.get("HEAVISIDE_VALIDATE_MPN") == "1":
        _target_mfr = str(getattr(state, "target_manufacturer", "") or "")
        for row in rows:
            ...
            if row.get("status") == "exact" and s_mpn.strip().lower() == str(
                ...
                continue
            s_mfr = str(row.get("substitute_manufacturer") or _target_mfr or "").strip()
            if not s_mfr:
                continue
...
def _mpn_datasheet_exists(
    ...
    OPT-IN: makes live network requests and the per-vendor datasheet-URL scheme is
    only validated for some families, so a real part on an unmodelled URL path
    could 404 and be wrongly demoted. Runs ONLY when HEAVISIDE_VALIDATE_MPN=1
    (default off) — never makes a surprise network call in tests or plain prod."""
    if not mpn or os.environ.get("HEAVISIDE_VALIDATE_MPN") != "1":
        return None
