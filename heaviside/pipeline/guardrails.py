...
_TAS_LOOKUP_CACHE: dict[tuple[str, str], Optional[dict]] = {}
            ...
            for _lineno, env in iter_envelopes(path):
                ...
                for top_key in ("capacitor", "semiconductor", "resistor",
                    ...
                    for inner_key in (None, "mosfet", "diode", "igbt"):
                        ...
                        cap_val = (cap_obj.get("nominal") if isinstance(cap_obj, dict)
                                   else cap_obj) if cap_obj is not None else None

                        res_obj = elec.get("resistance")
                        res_val = (res_obj.get("nominal") if isinstance(res_obj, dict)
                                   else res_obj) if res_obj is not None else None
...
def _g0_already_target_manufacturer(
    ...
    *,
    ...
    for comp in comps:
        ...
        comp["notes"] = (
            f"Already {target_manufacturer} part ({orig_pn}); verified in catalogue."
        )
...
def _g1_capacitor_value_mismatch(
    ...
    *,
    ...
    for comp in comps:
        ...
        kind_cap = (
            (bom_entry.get("type") or "").lower() == "capacitor"
            or (comp.get("type") or "").lower() == "capacitor"
        )
        if not kind_cap:
            continue
        ...
        if ratio < 0.5 or ratio > 2.0:
            ...
            fires.append(_make_fire(
                "1", ref, prev_status, "no_substitute",
    ...
    *,
    ...
    for comp in comps:
        ...
        if dev > 0.50 or sub_val / orig_val > 2.0 or orig_val / sub_val > 2.0:
            ...
            fires.append(_make_fire(
                "2a", ref, prev_status, "no_substitute",
...
def _g4_inductor_footprint_overrejection(
    ...
    """
    _FOOTPRINT_ONLY = (
        "package mismatch", "footprint mismatch", "not a drop-in",
        "requires pcb redesign", "package change", "footprint change",
    )
    _HARD_REJECTION = (
        r"isat", r"saturation", r"dcr.{0,30}exceed", r"value mismatch",
        r"extreme size", r"10x", r"20x",
    )
    ...
    for comp in comps:
        ...
        if kind not in ("inductor", "transformer", "common_mode_choke", "magnetic"):
            if comp_type not in ("inductor", "transformer", "common_mode_choke", "magnetic"):
                continue
    ...
    *,
    ...
    for comp in comps:
        ...
        if looks_like_description and not _WURTH_MPN_PREFIX_RE.match(pn):
            ...
            fires.append(_make_fire(
                "5b", ref, prev_status, "no_substitute",
        ...
        if not _mpn_exists_in_tas(pn, tas_data_dir=tas_data_dir):
            ...
            fires.append(_make_fire(
                "5", ref, prev_status, "no_substitute",
...
def _looks_automotive(mpn: str) -> bool:
    ...
    if any(u.startswith(p) for p in _AUTOMOTIVE_PREFIXES):
        return True
    if any(s in u for s in _AUTOMOTIVE_SUFFIXES):
        return True
    return False
...
def _gaecq_automotive_grade(
    ...
    *,
    ...
    """
    auto_context = any(
        _looks_automotive(s.get("original_pn") or "")
    ...
    for comp in comps:
        ...
        if not is_q:
            ...
            comp["notes"] = (
                ...
                f"carries no Q-grade in TAS. Verify qualification. "
                + (comp.get("notes") or "")
            )
            fires.append(_make_fire(
                "AECQ", ref, prev, "partial",
...
_SMD_SIZES = [
    ...
    "1812", "2010", "2220", "2512", "2920",
]
_LEADED_PKG_TOKENS = (
    "DIP", "PDIP", "TO-220", "TO-247", "TO-218", "TO-3P",
    "TO-126", "TO-92", "SIP", "RADIAL", "AXIAL", "THRU-HOLE",
    "THROUGH", "THT", "SNAP-IN", "SCREW-TERMINAL",
)
_SMD_PKG_TOKENS = (
    "0201", "0402", "0603", "0805", "1206", "1210", "1812",
    "2010", "2512", "2920", "DFN", "SON", "TDFN", "CSP",
    "SOT", "SOIC", "SOP", "DPAK", "D2PAK", "QFN", "BGA",
    "LGA", "SMA", "SMB", "SMC",
)
...
def _smd_class_idx(pkg: str) -> Optional[int]:
    if not pkg:
        return None
    p = pkg.upper()
...
def _gfoot_footprint_compatibility(
    ...
    *,
    ...
    for comp in comps:
        ...
        if ctype in ("inductor", "transformer", "common_mode_choke", "ferrite_bead",
                      "magnetic", "mosfet", "diode", "igbt"):
            continue
        ...
        # SMD <-> leaded check.
        if (_is_smd(src_pkg) and _is_leaded(sub_pkg)) or (
            _is_leaded(src_pkg) and _is_smd(sub_pkg)
        ):
            prev = comp.get("status")
            comp["status"] = "no_substitute"
            comp["substitute_pn"] = "no_substitute"
            comp["notes"] = (
                ...
                f"with substitute {sub_pkg}. "
                + (comp.get("notes") or "")
            )
            fires.append(_make_fire(
                "Foot", ref, prev, "no_substitute",
        ...
        if si is not None and ti is not None:
            ...
            if jump >= 4:
                ...
                comp["notes"] = (
                    ...
                    f"({jump} size classes — redesign required). "
                    + (comp.get("notes") or "")
                )
                fires.append(_make_fire(
                    "Foot", ref, prev, "no_substitute",
    ...
    for comp in comps:
        ...
        if guardrail_hits >= 2:
            ...
            comp["notes"] = (
                ...
                f"this is a redesign, not a substitution. "
                + (comp.get("notes") or "")
            )
            fires.append(_make_fire(
                "Stack", ref, prev, "no_substitute",
...
def _g7_voltage_stress(
    comps: list[dict], stress_by_ref: dict, fires: list[dict],
    *, tas_data_dir: Path | None = None,
) -> None:
    ...
    for comp in comps:
        ...
        if rated_i < stress.i_peak:
            ...
            fires.append({
                ...
                "after": "partial",
                "reason": (
                    f"sim I_peak={stress.i_peak:.2f}A, rated={rated_i:.1f}A"
                ),
            })
...
def apply_guardrails(
    ...
    *,
    ...
    _g0_already_target_manufacturer(comps, target_manufacturer, fires,
                                    tas_data_dir=tas_data_dir)

    _g1_capacitor_value_mismatch(comps, bom_by_ref, fires,
                                 tas_data_dir=tas_data_dir)

    _g2_resistor_value_drift(comps, bom_by_ref, fires,
                             tas_data_dir=tas_data_dir)

    _g3_capacitor_voltage_downrate(comps, bom_by_ref, fires,
                                   tas_data_dir=tas_data_dir)
    ...
    _gaecq_automotive_grade(comps, source_bom, fires,
                            tas_data_dir=tas_data_dir)

    _gfoot_footprint_compatibility(comps, bom_by_ref, fires,
                                   tas_data_dir=tas_data_dir)
