...
def main() -> None:
    ...
    design = design_converter(topology, spec, use_llm=True, with_reviewers=True,
                              sweep_kwargs={"f_lo_hz": 100_000, "f_hi_hz": 1_000_000,
                                            "n_coarse": 6, "golden_iters": 4,
                                            "top_k": 3, "max_candidates_per_fsw": 12},
                              progress=cb)
    ...
    o = design.outcome
    print("\n=== DESIGN ===")
    print(f"topology={design.topology}  fsw*={design.fsw_hz/1e3:.1f}kHz  verdict={design.verdict}")
    c = design.sweep.best
    print(f"magnetic (PINNED): {c.core_shape} / {c.core_material}  L={c.inductance_h*1e6:.1f}uH  "
    ...
    if gk:
        print(f"gatekeeper approved={gk.approved} objections={list(gk.objections)} "
    ...
    for row in design.bom:
        print(f"  {row['ref']:6s} {str(row.get('mpn')):24s} {str(row.get('manufacturer') or ''):18s} {row.get('category') or ''}")
    if design.notes:
        print("\nnotes:", "; ".join(design.notes)[:300])
    if o.diagnostics:
        print("diagnostics:", "; ".join(o.diagnostics)[:300])
    print("\n=== TIME / COST ===")
    print(f"wall-clock: {elapsed:.1f}s  ·  tokens calls={tok['calls']} in={tok['input']:,} out={tok['output']:,}")
