...
def main() -> None:
    ...
    print(f"  isat@100C={isat_s_100:.3f}A (margin {isat_s_100/ipeak_s:.2f})  isat@25C={isat_s_25:.3f}A (margin {isat_s_25/ipeak_s:.2f})")
    ...
    pick = TopologyPick(topology=entry, main_magnetic=md, candidates=(md,),
                        pick_reason="", pick_criteria="debug")
    ...
    for stage in tas.get("topology", {}).get("stages", []):
        for c in stage.get("circuit", {}).get("components", []):
            if isinstance(c, dict) and ("isat" in c or "ipeak_worst" in c):
                ...
                print(f"  comp={c.get('name')}: isat={c.get('isat')}A  ipeak_worst={c.get('ipeak_worst')}A "
                      f"margin={ (c.get('isat')/c.get('ipeak_worst')) if c.get('isat') and c.get('ipeak_worst') else '?'}")
                prov = c.get("isat_provenance") or {}
                print(f"    isat_provenance: T={prov.get('temperature_c')}C method={prov.get('method')}")
    if not found:
        print("  (no magnetic component with isat/ipeak found)")
    # what L did the gate use?
    print(f"  spec desiredInductance passed to gate: see decompose; harvested L_s={L_s*1e6:.2f}uH")
    # realism verdict
    vd = outcome.verdict_dict or {}
    for c in vd.get("checks", []):
        if c.get("name") == "inductor_isat_margin":
            print(f"  realism inductor_isat_margin: status={c.get('status')} value={c.get('value')} margin={c.get('margin')}")
