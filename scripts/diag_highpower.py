...
for k, v in (sim.items() if isinstance(sim, dict) else []):
    if isinstance(v, dict) and "efficiency_analyst" in v:
        print(f"  eta_analyst={v.get('efficiency_analyst')} pin={v.get('pin_analyst')} ploss={v.get('total_loss_analyst')}", flush=True)
...
for _stage, comp in _iter_components(tas):
    ...
    for f in ("rds_on", "vds_rated", "vds_stress", "id_rated", "id_stress",
              "rth_ja", "tj", "tj_max", "isat", "ipeak_worst", "qg_total"):
        if f in comp:
