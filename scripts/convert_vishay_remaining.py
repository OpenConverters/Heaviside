...
def make_inductor_document(product: dict) -> dict[str, Any] | None:
    ...
    if dcr is not None:
        doc["magnetic"]["manufacturerInfo"]["datasheetInfo"]["electrical"]["dcResistance"] = {"maximum": dcr}
    if rated_current is not None:
        doc["magnetic"]["manufacturerInfo"]["datasheetInfo"]["electrical"]["ratedCurrent"] = rated_current
    if sat_current is not None:
        doc["magnetic"]["manufacturerInfo"]["datasheetInfo"]["electrical"]["saturationCurrentPeak"] = sat_current
...
def main():
    ...
    total_inductors += process_file("Inductors - Transformers.html", "magnetics.ndjson", make_inductor_document)
    
    # Process MOSFETs
    print("\n--- MOSFETs ---")
    total_mosfets += process_file("MOSFETs - Automotive.html", "mosfets.ndjson", make_mosfet_document)
    total_mosfets += process_file("mosfets.html", "mosfets.ndjson", make_mosfet_document)
    
    # Process Diode Modules
    print("\n--- DIODE MODULES ---")
    total_diodes += process_file("Modules - Modules, Diode - FRED Pt®.html", "diodes.ndjson", make_diode_document)
    total_diodes += process_file("Modules - Modules, Diode - HEXFRED®.html", "diodes.ndjson", make_diode_document)
    total_diodes += process_file("Modules - Modules, Diode - High Performance Schottky.html", "diodes.ndjson", make_diode_document)
    total_diodes += process_file("Modules - Modules, Diode - High Voltage.html", "diodes.ndjson", make_diode_document)
    total_diodes += process_file("Modules, Diode - Silicon Carbide (SiC).html", "diodes.ndjson", make_diode_document)
    ...
    print("\n--- MOSFET MODULES ---")
    total_mosfets += process_file("Modules - Modules, MOSFET.html", "mosfets.ndjson", make_mosfet_document)
