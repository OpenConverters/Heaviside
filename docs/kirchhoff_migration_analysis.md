...
## Update — 2026-06-24: closed-loop landed in Kirchhoff (H6 resolved)

Kirchhoff added closed-loop control (PFC/Vienna active current + outer voltage loops, "AC control hardening", an LTspice second backend, parameter sweeps, and an **independent averaged-model oracle** at `tests/averaged_model.py`) plus a **requirements gate** (`tests/test_requirements.cpp`): every topology must now **DELIVER ITS SPEC** — settled Vout within **±5%** (`kReqTol`), power ±10% — which is *stronger than, and supersedes,* the MKF-equivalence check (Kirchhoff now regulates/designs to the target rather than reproducing MKF's open-loop operating point). TAS-side, the control stage is the CTAS `controller.json`. This **resolves gap H6** ("no closed-loop in Kirchhoff") from §4.

**Effect on the bound topologies (fresh `PyKirchhoff.so`, 06-24):** boost now delivers **23.87 V (0.53% off 24)** and flyback **11.96 V (0.32% off 12)** through HS's Kirchhoff backend — vs the old open-loop 23.11 / 11.13.

**HS adaptation done:** the cross-backend harness (`tests/unit/test_kirchhoff_equivalence.py`) was **rewritten from "match MKF's open-loop reference" to a delivers-spec gate** that mirrors Kirchhoff's requirements gate — asserting the Kirchhoff backend delivers each bound topology's spec Vout within ±5%. (The old reference comparison is obsolete now that Kirchhoff intentionally diverges from MKF's open-loop point to hit the target.) The `spice_sim` self-contained-deck runner already executes whatever deck Kirchhoff emits (open- or closed-loop) and parses its `.meas` Vout, so no seam change was needed.

**Revised next steps:** (1) bind the remaining ~18 PyKirchhoff designers (incl. the AC PFC/Vienna closed-loop ones) in `src/bindings.cpp` + `ninja -j3` → the delivers-spec gate auto-covers them; (2) wire `spice_sim` into `stage3_realize` (gap G15); (3) for AC/PFC topologies, the delivers-spec gate should also assert power-factor / regulation where the reference fixtures expose it. Consider adopting Kirchhoff's averaged-model oracle as an HS-side independent cross-check of the SPICE result.