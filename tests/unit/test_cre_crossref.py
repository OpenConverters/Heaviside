...
class TestReferenceSpec:
    def test_to_heaviside_spec(self) -> None:
        ref = ReferenceSpec(
            topology="flyback",
            vin_min=85, vin_nom=230, vin_max=265,
            vout=20, iout=3.25, pout=65, fsw=65000,
    ...
    def test_no_optional_fields(self) -> None:
        ref = ReferenceSpec(
            topology="buck",
            vin_min=36, vin_nom=48, vin_max=60,
...
_MOCK_RE_RESPONSE = json.dumps({
    "topology": "buck",
    "specs": {
        "topology": "buck", "vin_min": 36, "vin_max": 60,
        "vout": 12, "iout": 5, "pout": 60, "fsw": 200000,
    },
    "bom": [
        {"ref_des": "Q1", "role": "primarySwitch", "mpn": "CSD19536KTT",
         "category": "mosfet", "package": "TO-220", "value": ""},
        {"ref_des": "D1", "role": "freewheelDiode", "mpn": "SS34",
         "category": "diode", "package": "DO-214AB", "value": ""},
    ],
})
        ...
        state = CrossRefState(
            source_bom=[
                {"ref_des": "Q1", "component_type": "mosfet",
                 "mpn": "CSD19536KTT", "manufacturer": "TI"},
            ],
