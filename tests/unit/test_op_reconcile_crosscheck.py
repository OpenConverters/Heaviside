...
def _est(i, ipeak, isat, tj=None, tj_max=None):
    return orc.OpEstimate(
        op_index=i, ipeak_a=ipeak, isat_a=isat, tj_c=tj, tj_max_c=tj_max, label=f"op{i}"
    )
...
def test_zvs_widens_efficiency_tolerance():
    ...
    est = [
        cc.Estimate("efficiency", 0.98, "analyst"),
...
def test_realism_estimators_agree_unavailable_without_crosscheck():
    ...
    tas = {"topology": {"stages": []}}
    assert (
        _check(evaluate_tas(tas, topology="buck"), "estimators_agree").status
